...
export default function DashboardProductWarehouse({ orders }) {
  const productWarehouseOrders = orders.filter(o => o.step === 6);
  const myTasks = productWarehouseOrders.filter(o => o.status !== "Hoàn tất");
  const today = new Date().toISOString().slice(0, 10);
  const lateOrders = productWarehouseOrders.filter(o => o.deliveryDate < today && o.status !== "Hoàn tất");
  const last10Orders = productWarehouseOrders.sort((a, b) => b.id - a.id).slice(0, 10);
  const [selected, setSelected] = useState(null);

  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <div className="text-3xl font-bold mb-4">Dashboard phòng Kho thành phẩm</div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          <div className="text-3xl">🏪</div>
          <div>
            <div className="text-gray-500 text-sm">Đơn phòng Kho thành phẩm</div>
            <div className="text-2xl font-bold">{productWarehouseOrders.length}</div>
          </div>
        </div>
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          <div className="text-3xl">⏰</div>
        ...
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold">{productWarehouseOrders.filter(o => o.status === "Hoàn tất").length}</div>
          </div>
        </div>
      </div>
      {/* Việc hôm nay */}
      ...
      <div>
        <div className="text-xl font-semibold mb-2">10 đơn gần nhất phòng Kho thành phẩm</div>
        <div className="bg-white rounded-2xl shadow p-4">
          <table className="w-full">
            <thead>
              <tr className="text-left text-gray-500">
