...
export default function DashboardDesign({ orders }) {
  // Lọc đơn thuộc phòng Thiết Kế (step = 0 hoặc theo logic thực tế)
  const designOrders = orders.filter(o => o.step === 0);

  // Việc hôm nay: những đơn chưa hoàn thành ở bước Thiết kế
  const myTasks = designOrders.filter(o => o.status !== "Hoàn tất");

  // Đơn trễ (ví dụ: ngày giao đã qua hôm nay)
  const today = new Date().toISOString().slice(0, 10);
  const lateOrders = designOrders.filter(o => o.deliveryDate < today && o.status !== "Hoàn tất");

  // 10 đơn gần nhất của phòng thiết kế (sắp xếp theo ngày tạo/ID giảm dần)
  const last10Orders = designOrders
    .sort((a, b) => b.id - a.id)
    .slice(0, 10);

  // State hiển thị chi tiết đơn
  const [selected, setSelected] = useState(null);

  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <div className="text-3xl font-bold mb-4">Dashboard phòng Thiết Kế</div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          <div className="text-3xl">🎨</div>
          <div>
            <div className="text-gray-500 text-sm">Đơn phòng Thiết Kế</div>
            <div className="text-2xl font-bold">{designOrders.length}</div>
          </div>
        </div>
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          <div className="text-3xl">⏰</div>
        ...
        <div className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold">{designOrders.filter(o => o.status === "Hoàn tất").length}</div>
          </div>
        </div>
      </div>
      {/* Việc hôm nay */}
      ...
      <div>
        <div className="text-xl font-semibold mb-2">10 đơn gần nhất phòng Thiết Kế</div>
        <div className="bg-white rounded-2xl shadow p-4">
          <table className="w-full">
            <thead>
              <tr className="text-left text-gray-500">
